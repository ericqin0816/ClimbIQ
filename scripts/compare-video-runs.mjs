...
import { compareVideoRuns, parseComparisonArgs } from "./lib/compare-video-runs.mjs";

const { beforePath, afterPath, toleranceSeconds } = parseComparisonArgs(process.argv.slice(2));
const [before, after] = await Promise.all([beforePath, afterPath].map(async file => JSON.parse(await readFile(file, "utf8"))));
const result = compareVideoRuns(before, after, toleranceSeconds);
