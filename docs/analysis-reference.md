...
[Project overview](../README.md) · [Live app](https://climbiq-detection-lab.vercel.app) · [Evaluation notes](../REAL_VIDEO_BENCHMARK.md)

Implementation details and operating instructions for ClimbIQ. For a first run,
...
Audio decoding requests the detector's 8 kHz sample rate, with a default-rate fallback for browsers that reject it. In local Chrome tests this reduced decoded PCM allocation about 5.5-fold without changing the six original user-video decisions; it is not a whole-app memory or speed guarantee. Decoding still reads the complete encoded file. File, device and codec failures return missing audio evidence, while cancellation remains an explicit cancellation. The detector's overlapping 20 ms windows are not sample-accurate event labels. See [audio experiment and limitations](../benchmarks/AUDIO_DECODE_2026_09_05.md); with a local dev server running, `npm run test:audio-browser` checks the real browser decoder using synthetic audio.

Before accepting a clock, Quick Analyze checks lane-local body motion and full-frame scene continuity. It requires a reliable launch after the cue and rejects camera cuts or reframing. Motion already underway, a suspicious first sample, a delay below the app's 0.100 s visual-audit floor, or an implausibly late launch requires review. Visible motion is not electronic pad release, and the app does not make false-start rulings. Light/audio provide the timing candidate; body motion and scene continuity guard against false acceptance.
